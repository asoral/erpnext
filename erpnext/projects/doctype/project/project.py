...
class Project(Document):
				...
				for task in template.tasks:
					if task in req:
						...
						if not template_task_details.parent_task and template_task_details.is_group==1:
							...
							task = self.create_task_from_template(template_task_details,req)
							if task:
						...
						if template_task_details.parent_task and template_task_details.is_group==0:
							...
							if task:
								project_tasks.append(task)
					self.dependency_mapping(tmp_task_details, project_tasks,req)
			else:
				for task in template.tasks:
					template_task_details = frappe.get_doc("Task", task.task)
					tmp_task_details.append(template_task_details)
	...
	def create_task_from_template(self, task_details,req):
		if self.scope_of_supply:
			if task_details.name in req:
				taskdoc= frappe.get_doc(dict(
					...
					exp_start_date = self.calculate_start_date(task_details,req),
					exp_end_date = self.calculate_end_date(task_details,req),
	...
	def calculate_end_date(self, task_details,req):
		if self.scope_of_supply:
			if task_details.name in req:
	...
	def dependency_mapping(self, template_tasks, project_tasks,req):
		if self.scope_of_supply:
			for template_task in template_tasks:
				if template_task.name in req:
					...
					self.check_depends_on_value(template_task, project_task, project_tasks,req)
					self.check_for_parent_tasks(template_task, project_task, project_tasks,req)
		else:
			for template_task in template_tasks:
				project_task = list(filter(lambda x: x.subject == template_task.subject, project_tasks))[0]
				project_task = frappe.get_doc("Task", project_task.name)
	...
	def check_depends_on_value(self, template_task, project_task, project_tasks,req):
		if self.scope_of_supply:
			if template_task.name in req:
	...
	def check_for_parent_tasks(self, template_task, project_task, project_tasks,req):
		if self.scope_of_supply:
			if template_task.name in req:
