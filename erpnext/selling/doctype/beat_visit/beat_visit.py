...
class BeatVisit(Document):
	...
	def on_submit(self):
		...
		if self.pymt>0:
			get_acc = frappe.get_doc('Company',self.company)
			cash_acc = get_acc.default_cash_account
			bank_acc = get_acc.default_bank_account
			curr = get_acc.default_currency
	...
	def cust_stats(self):
		...
		if last_date:
			self.last_visit_date = last_date
		else:
			self.last_visit_date = 'Not Visited'
		...
		if last_order_date:
			...
			self.last_order_date = 'No Previous Order'
