...
def remove_authorize(user, password, action, pos_profile,company, owner, item, canceled_transaction):
	...
	for d in docs:
		...
		print(" this is supervisor ", supervisor, canceled_transaction)
		if supervisor.user == user and password == supervisor.get_password("password"):
			print(" this is true users 1111111111111111111")
			...
			doc.canceled_transaction = canceled_transaction
			doc.save(ignore_permissions = 1)
			return True	
			
		if canceled_transaction:
			print(" this is transcations", canceled_transaction)
			pos_invoice = frappe.get_doc("POS Invoice", canceled_transaction)
			print(" this is pos inv", pos_invoice)
			if pos_invoice:
				print(" this is pos inv", pos_invoice)
				frappe.db.set_value('POS Invoice', pos_invoice.name , 'docstatus', 2, update_modified=False)
...
def editcart_authorize(user, password, action, company,pos_profile, owner, item, canceled_transaction):	
	...
	for d in docs:
		supervisor = frappe.get_doc("POS Profile Supervisor", d)
		print(" this is supervisor ", supervisor, canceled_transaction)
		if supervisor.user == user and password == supervisor.get_password("password") and canceled_transaction:
			print(" this is true users 2222222222222222222 ")
			doc = frappe.new_doc("Supervisor Log")
			doc.user = user
			doc.action = action
			doc.date_time = now_datetime()
