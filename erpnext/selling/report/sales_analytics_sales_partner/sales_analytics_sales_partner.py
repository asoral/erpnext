...
class Analytics(object):
	...
	def get_sales_transactions_based_on_customer_or_territory_group(self):
		...
		if self.filters.tree_type == 'Customer Group':
			entity_field = 'customer_group as entity_field'
		else:
			entity_field = "territory as entity_field"
		...
		if self.filters.sales_person:
			self.entries =frappe.db.sql("""
				...
				and s.{date_field} between %s and %s
				"""
			.format(date_field=self.date_field,entity_field=entity_field, value_field=value_field, doctype=self.filters.doc_type),
			(self.filters.company,self.filters.sales_person, self.filters.from_date, self.filters.to_date), as_dict=1)
		else:
			self.entries = frappe.get_all(self.filters.doc_type,
