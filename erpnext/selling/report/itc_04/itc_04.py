...
def get_data(filters,conditions):
	...
	if filters.report == "ITC-05 A":
		...
		# 			where
		# 			pr.docstatus = 1 and se.stock_entry_type = "Send to Subcontractor" and se.docstatus = 1 and
		...
		# 	print("-------------------dist_pr_batch",dfrappe.db.sql(""" select adds.gstin as gstin_of_job_worker,
		# 												adds.state as state, supp.gst_category as job_workers_type
		# 												from `tabSupplier` supp
		# 												INNER JOIN `tabDynamic Link` dl
		# 												on dl.link_name = supp.name
		...
		# 												on dl.parent = adds.name
		# 												where supp.name = %(supp)s """,
		# 											 {'supp': pr_doc.supplier}, as_dict=1)ist_pr_batch)
		...
		# 			list_items=[]
		# 			item_list = frappe.get_list("Stock Entry Detail", filters={ 'parent': se_doc.name}, fields=['item_code'])
		# 			print(' item code 1', item_list)
		# 			for l in item_list:
		# 				print(" l ", l.item_code)
		# 				list_items.append(l.get('item_code'))
		# 			print(" istffff", list_items)	
		# 			if row.qty_to_be_consumed > 0 and row.rm_item_code in list_items:
