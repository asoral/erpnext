...
class FiscalYear(Document):
	...
	def validate_dates(self):
		...
		if getdate(self.year_start_date) > getdate(self.year_end_date):
			frappe.throw(
				frappe._("Fiscal Year Start Date should be one year earlier than Fiscal Year End Date"),
		...
		if getdate(self.year_end_date) != date:
			frappe.throw(
				frappe._("Fiscal Year End Date should be one year after Fiscal Year Start Date"),
				FiscalYearIncorrectDate,
			)
	...
	def validate_overlap(self):
		...
		if existing_fiscal_years:
			for existing in existing_fiscal_years:
				...
				if overlap:
					frappe.throw(frappe._("Year start date or end date is overlapping with {0}. To avoid please set company")
...
def check_duplicate_fiscal_year(doc):
	...
	for fiscal_year, ysd, yed in year_start_end_dates:
		if (getdate(doc.year_start_date) == ysd and getdate(doc.year_end_date) == yed) and (
			...
			frappe.throw(
				frappe._("Fiscal Year Start Date and Fiscal Year End Date are already set in Fiscal Year {0}").format(
					fiscal_year
				)
			)
